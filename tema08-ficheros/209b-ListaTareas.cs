...
class Program
{
    ...
    public static void Guardar(List<Tarea> tareas, string nombreFichero)
    {
        try
        {
            ...
            {
                escribir.WriteLine(t.Descripcion);
                escribir.WriteLine(t.Prioridad);
                escribir.WriteLine(t.Completada);
            }
            escribir.Close();
        } 
        catch(IOException)
    ...
    public static void Cargar(List<Tarea> tareas, string nombreFichero)
    {
        if(File.Exists(nombreFichero))
        {
            try
            {
                ...
                {
                    t.Descripcion = lectura.ReadLine();
                    t.Prioridad = Convert.ToInt32(lectura.ReadLine());
                    t.Completada = Convert.ToBool(lectura.ReadLine());
                }
                lectura.Close();
            }
            catch(IOException)
            {
                Console.WriteLine("Error al intentar cargar los cambios");
            }
            catch(FormatException)
            {
                Console.WriteLine("Datos cargados corruptos");
            }
            catch(Exception)
            {
