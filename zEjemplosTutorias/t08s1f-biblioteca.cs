// Crea una nueva versión un ejercicio con lista de
// struct o de class (por ejemplo, el 190 - Biblioteca),
// haciendo que se guarden los datos en fichero al terminar
// la ejecución y que se carguen desde fichero (si existe)
...
class Ejercicio190
{
    ...
    static void Guardar(List<Libro> libros)
    {
        try
        {
            StreamWriter f = File.CreateText("libros.txt");
            foreach (Libro l in libros)
            {
                f.WriteLine(l.titulo + "#" + l.autor + "#" + l.anyoPubli);
            }
            f.Close();
        }
    ...
    static List<Libro> Cargar()
    {
        List<Libro> libros = new List<Libro>();
        if (File.Exists("libros.txt"))
        {
            string linea;
            StreamReader fichero = File.OpenText("libros.txt");
            do
            {
                ...
                if (linea != null)
                {
                    ...
                    Libro nuevoLibro;
                    nuevoLibro.titulo = fragmentos[0];
                    nuevoLibro.autor = fragmentos[1];
                    nuevoLibro.anyoPubli = Convert.ToInt16(fragmentos[2]);
                    libros.Add(nuevoLibro);
                }
            } while (linea != null);
            fichero.Close();
