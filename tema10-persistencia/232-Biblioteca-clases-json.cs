...
Crea una nueva versión del ejercicio de la biblioteca + List 
(ejercicio anterior, o, si no lo has conseguido, la versión 
original 190), en la que uses serialización JSON para cargar 
datos al comienzo de la sesión y para guardar los datos tras 
cada modificación.
...
public class ListaDeLibros
{
    ...
    private string NombreFich = "libros.json";
    public int Cantidad
    {
    ...
    private ListaDeLibros Cargar()
    {
        ...
        {
            string jsonString = File.ReadAllText(NombreFich);
            ListaDeLibros lista = JsonSerializer.Deserialize<ListaDeLibros>(jsonString);
            return lista;
        }
        else
        {
    ...
    private static void Guardar(string nombreFich, List<Libro> libros)
    {
        ...
        File.WriteAllText(nombreFich, jsonString);
    }
